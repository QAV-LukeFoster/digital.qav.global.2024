...
class listingController
{
  ...
  public function __construct($module,$url_params = '',$sf_request,$sf_user)
	...
	public function insert_button($title)
	{
      ...
      return '<button class="btn btn-success" onClick="openModalBox(\'' . url_for($this->module . '/new' . $this->add_url_params('?'),true) . '\')">' . $title . '</button>';
    }
  }
  ...
  public function edit_button($id,$projects_id=0)
	{
	  if($this->access['edit'])
	  {  
      return '<a href="#" class="btn btn-warning btn-xs" onClick="openModalBox(\'' . url_for($this->module . '/edit?id=' . $id  . $this->add_projects_id($projects_id) . $this->add_url_params('&')). '\'); return false;"><i class="fa fa-edit"></i></a>';
    }
  }
  ...
  public function delete_button($id,$projects_id=0,$confirm = false)
	{
      ...
      return link_to('<i class="fa fa-trash-o"></i>', $this->module  . '/delete?id='  . $id  . $this->add_projects_id($projects_id) . $this->add_url_params('&'), array('method' => 'delete','class'=>'btn btn-danger btn-xs', 'confirm' => $confirm));
    }
  }
